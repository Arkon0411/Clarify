...
export function TaskCard({ task, isDragging, showCheckbox = false }: TaskCardProps) {
  ...
  return (
    <div
      className={cn(
        "rounded-xl bg-card p-3 sm:p-4 shadow-sm border border-border transition-all",
        isDragging ? "shadow-lg ring-2 ring-primary/20" : "hover:shadow-md",
        isChecked && showCheckbox && "bg-sage-light/20 border-sage/30",
      )}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-2 sm:mb-3">
            <h4 className={cn(
              "text-xs sm:text-sm font-medium text-foreground leading-snug text-pretty flex-1 min-w-0",
              isChecked && showCheckbox && "line-through text-muted-foreground"
            )}>
              {task.title}
            </h4>
            <span
